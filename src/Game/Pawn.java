...
public class Pawn implements ChessPiece {
	...
	Image imgWhite = new ImageIcon(getClass().getResource("/Assets/PawnBlack.png")).getImage();
	Image imgBlack = new ImageIcon(getClass().getResource("/Assets/PawnWhite.png")).getImage();
	int x = 0;
	int y = 0;
	int gridX = 0;
	int gridY = 0;
	...
	public Pawn(ChessBoard b, int x, int y, int side) {
		this.b = b;
	...
	public List<Move> getValidMoves(ChessBoard b) {
		...
		if(firstMove && !b.pieceAt(gridX, gridY-this.side)) {
			moves.add(new Move(gridX, gridY, gridX, gridY-(2*this.side)));
		}
		if(!b.pieceAt(gridX, gridY-this.side)) {
			moves.add(new Move(gridX, gridY, gridX, gridY-this.side));
		}
		if(b.pieceAt(gridX-1, gridY-this.side)) {
			moves.add(new Move(gridX, gridY, gridX-1, gridY-this.side));
		}
		if(b.pieceAt(gridX+1, gridY-this.side)) {
			moves.add(new Move(gridX, gridY, gridX+1, gridY-this.side));
		}
