...
public class Queen implements ChessPiece {
	...
	Image imgWhite = new ImageIcon(getClass().getResource("/Assets/QueenBlack.png")).getImage();
	Image imgBlack = new ImageIcon(getClass().getResource("/Assets/QueenWhite.png")).getImage();
	int x = 0;
	int y = 0;
	int gridX = 0;
	int gridY = 0;
	...
	public Queen(ChessBoard b, int x, int y, int side) {
		this.b = b;
	...
	private ArrayList<Move> castLine(int dx, int dy) {
		ArrayList<Move> retVal = new ArrayList<Move>();
		for(int i = 1; i < 8; i ++) {
			retVal.add(new Move(gridX, gridY, gridX+(dx*i), gridY+(dy*i)));
			if(b.pieceAt(gridX+(dx*i), gridY+(dy*i))) break;
		}
	...
	public List<Move> getValidMoves(ChessBoard b) {
		...
		moves.addAll(this.castLine(0, 1));
		moves.addAll(this.castLine(1, 0));
		moves.addAll(this.castLine(0, -1));
		moves.addAll(this.castLine(-1, 0));
		moves.addAll(this.castLine(1, 1));
		moves.addAll(this.castLine(-1, -1));
		moves.addAll(this.castLine(-1, 1));
		moves.addAll(this.castLine(1, -1));
